...
{
    public sealed class NgramList<T> where T : class, INgram
    {
        ...
        public T? GetNgram()
        {
            ...
            {
                ngram = x,
                likeness = NgramList<T>.WordLikeness(x.ToList().Aggregate((a, b) => a + b), reference.Aggregate((a, b) => a + b))
            })
        ...
        private static int WordLikeness(string word, string reference)
        {
            int likeness = 0;
            int startIndex = 0;
